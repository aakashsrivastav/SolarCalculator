...
open class GooglePlacesRequestHelpers {
    ...
    static func reverseGeocode(latitude: Double, longitude: Double, apiKey: String, completion: @escaping (PlaceDetails?) -> Void) {
        var parameters = [ "latlng": "\(latitude),\(longitude)", "key": apiKey ]
        if let deviceLanguage = deviceLanguage {
            parameters["language"] = deviceLanguage
        }
        doRequest(
            ...
            params: parameters,
            completion: { jsonDict in
                if let array = jsonDict["results"] as? [[String: Any]],
                    let dict = array.first {
                    completion(PlaceDetails(json: ["result": dict]))
                } else {
                    completion(PlaceDetails(json: jsonDict as? [String: Any] ?? [:]))
                }
        })
    }
